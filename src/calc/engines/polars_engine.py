...
class PolarsEngine:
    ...
    def filter_candidates(
        ...
        if "volume" in df.columns:
            ldf = ldf.filter(
                (pl.col("volume") >= min_volume) | pl.col("volume").is_null()
            )

        # 2. 財務安全性フィルター（極端な財務危機銘柄を一次排除）
        if "equity_ratio" in df.columns:
            ldf = ldf.filter(
                (pl.col("equity_ratio") >= min_equity_ratio)
    ...
    def _load_thresholds(self):
        ...
        try:
            ...
            self.logger.warning(
                f"⚠️ Failed to load thresholds from {THRESHOLDS_PATH}: {e}"
            )
            self.thresholds = {}
    ...
    def calculate_scores(
        self, df: pl.DataFrame, strat_cfg: Optional[dict[str, Any]] = None
    ) -> pl.DataFrame:
        ...
        self.logger.info(
            f"🚀 PolarsEngine: Start calculating {len(df)} rows for '{self.strategy_name}'"
        )
        ...
        wf, wt = (
            float(weights.get("weight_fund", 0.7)),
            float(weights.get("weight_tech", 0.3)),
        )
        ...
        linear_candidates = {
            "roe": (5.0, 15.0),
            "per": (15.0, 10.0),
            "payout_ratio": (30.0, 100.0),
            "dividend_yield": (2.0, 4.0),
            "sales_growth": (5.0, 20.0),
            "profit_growth": (5.0, 30.0),
        }
        ...
        for metric, weight in points_cfg.items():
            ...
            if metric in linear_candidates:
                ...
                pts_expr = self._build_linear_score_expr(
                    metric, m_min, m_target, pts_val
                )
            else:
            ...
            if metric == "dividend_yield" and "payout_ratio" in df.columns:
                ...
                pts_expr = (
                    pl.when(condition & (payout < 100.0))
                    .then(pts_expr)
                    .when(condition & (payout >= 100.0))
                    .then(pts_expr * 0.5)
                    .otherwise(0.0)
                )
        ...
        if "macd_hist" in df.columns:
            ...
            category_map["trend"].append(
                pl.when(m_hist > 0)
                .then(5.0 + (m_hist * 2.0).clip(0, 5.0))
                .when(m_hist < 0)
                .then(-5.0 + (m_hist * 2.0).clip(-5.0, 0))
                .otherwise(0.0)
            )
        if "rsi_14" in df.columns:
            category_map["trend"].append(
                ((50.0 - pl.col("rsi_14").fill_null(50.0)) * 0.5).clip(-15.0, 15.0)
            )
        ...
        for col_name, bonus_map in status_bonuses.items():
            ...
            for val_key, pts in bonus_map.items():
                ...
                cond = (pl.col(col_name).cast(pl.Utf8) == v_str) | (
                    pl.col(col_name).cast(pl.Utf8) == v_str.replace(".0", "")
                )
                expr = pl.when(cond).then(pts_f).otherwise(0.0)
                bonus_all_list.append(expr)
                if pts_f > 0:
                    bonus_growth_list.append(expr)
                elif pts_f < 0:
                    bonus_penalty_list.append(expr.abs())

        # 2. パイプライン・チェーン定義
        # -- Layer 1: 水平集計 --
        ldf = ldf.with_columns(
            [
                pl.sum_horizontal(category_map["base"])
                .fill_null(0.0)
                .alias("score_base"),
                pl.sum_horizontal([pl.lit(0.0)] + category_map["value"])
                .fill_null(0.0)
                .alias("score_value"),
                pl.sum_horizontal(
                    [pl.lit(0.0)] + category_map["growth"] + bonus_growth_list
                )
                .fill_null(0.0)
                .alias("score_growth"),
                pl.sum_horizontal([pl.lit(0.0)] + category_map["quality"])
                .fill_null(0.0)
                .alias("score_quality"),
                pl.sum_horizontal(
                    [pl.lit(0.0)] + category_map["penalty"] + bonus_penalty_list
                )
                .fill_null(0.0)
                .alias("score_penalty"),
                pl.sum_horizontal([pl.lit(0.0)] + category_map["trend"])
                .fill_null(0.0)
                .alias("score_trend"),
                pl.sum_horizontal(
                    [pl.lit(0.0)] + category_map["final_adjustment"] + bonus_all_list
                )
                .fill_null(0.0)
                .alias("score_final_adj"),
            ]
        )
        ...
        ldf = ldf.with_columns(
            [
                (
                    ...
                    / pl.lit(max(self.theory_max["fund"], 1.0))
                    * 100.0
                )
                .fill_null(0.0)
                .clip(0, 100)
                ...
                (
                    pl.col("score_trend")
                    / pl.lit(max(self.theory_max["trend"], 1.0))
                    * 100.0
                )
                .fill_null(0.0)
                .clip(0, 100)
        ...
        ldf = ldf.with_columns(
            [
                (
                    (
                        (pl.col("fund_norm") * wf)
                        + (pl.col("trend_norm") * wt)
                        + pl.col("score_final_adj")
                    )
                    * size_mult
                )
                .fill_null(0.0)
        ...
        final_ldf = (
            ...
            .with_columns(
                [
                    (
                        pl.col("quant_score_normalized").fill_null(0.0) + noise_expr
                    ).alias("raw_with_noise")
                ]
            )
            .with_columns(
                [
                    pl.col("raw_with_noise")
        ...
        if "rsi_14" in df.columns:
            ...
            final_ldf = final_ldf.with_columns(
                [
                    pl.when(is_dead)
                    .then(pl.col("quant_score").clip(0, 50.0))
                    .otherwise(pl.col("quant_score"))
                    .alias("quant_score")
                ]
            )
        ...
        if purged:
            self.logger.debug(
                f"Protocol: Purged internal calculation clutter: {purged}"
            )
    ...
    def _ensure_numeric(self, df: pl.DataFrame, strat_cfg: dict) -> pl.DataFrame:
        targets = list(strat_cfg.get("points", {}).keys()) + [
            "macd_hist",
            "rsi_14",
            "market_cap",
            "payout_ratio",
    ...
    def _get_threshold(self, metric: str) -> float | None:
        ...
        if metric in strat_thresholds:
            return float(strat_thresholds[metric])
        if metric in self.thresholds:
            return float(self.thresholds[metric])
        for cat_vals in self.thresholds.values():
            if isinstance(cat_vals, dict) and metric in cat_vals:
                return float(cat_vals[metric])
        return None

    def _determine_direction(self, metric: str, direction_meta: str | None) -> bool:
        if direction_meta == "lower":
            return True
        if direction_meta == "higher":
            return False
