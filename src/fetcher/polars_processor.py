...
class PolarsProcessor:
    ...
    def clean_anomalous_prices(df: pl.DataFrame | pd.DataFrame) -> pl.DataFrame:
        ...
        price_col = "price" if "price" in df_pl.columns else ("Close" if "Close" in df_pl.columns else None)
        if not price_col:
            return df_pl

        # 1. ゼロ以下の価格・負の出来高を Null に置換
        cleaned = df_pl.with_columns(
            pl.when(pl.col(price_col) <= 0).then(None).otherwise(pl.col(price_col)).alias(price_col)
        )
        if "Volume" in cleaned.columns:
            cleaned = cleaned.with_columns(
                pl.when(pl.col("Volume") < 0).then(0).otherwise(pl.col("Volume")).alias("Volume")
            )
        if "volume" in cleaned.columns:
            cleaned = cleaned.with_columns(
                pl.when(pl.col("volume") < 0).then(0).otherwise(pl.col("volume")).alias("volume")
    ...
    @staticmethod
    def pad_calendar(df: pl.DataFrame, start_date: Any = None, end_date: Any = None) -> pl.DataFrame:
        ...
        try:
            ...
            if not calendar_dates:
                # キャッシュミス：外部APIから取得
                logger.info(f"Calendar cache miss for {s_date} to {e_date}. Fetching from mcal...")
            ...
            df_padded = (
                ...
                .with_columns([
                    pl.all().exclude(["entry_date", "code"]).forward_fill().over("code")
                ])
            )
            return df_padded
    ...
    def _get_rsi_expr(window: int = 14, stability_min: int = 30) -> List[pl.Expr]:
        ...
        valid_count = pl.col("price").is_not_null().cast(pl.Int32).rolling_sum(stability_min).over("code")
        
        # 3. Wilder's Smoothing (EWMA)
        avg_gain = gain.fill_null(0.0).ewm_mean(com=window-1, adjust=False).over("code")
        avg_loss = loss.fill_null(0.0).ewm_mean(com=window-1, adjust=False).over("code")
        ...
        return [
            pl.when(valid_count.fill_null(0) < stability_min).then(None)
            .when(avg_loss == 0)
                ...
                for dt_col in ["Date", "index", "entry_date"]:
                    if dt_col in part.columns:
                        if part.schema[dt_col] == pl.Utf8:
                            part = part.with_columns(pl.col(dt_col).str.to_datetime(strict=False))
                        try:
                            # Use strict=False to handle slightly malformed strings gracefully
                            part = part.with_columns(pl.col(dt_col).cast(pl.Datetime("ns"), strict=False))
                        except Exception:
                            part = part.with_columns(pl.col(dt_col).cast(pl.Datetime("us"), strict=False))
    ...
    def _ensure_resilient_schema(df: pl.DataFrame) -> pl.DataFrame:
        ...
        return df.select([pl.col(c).cast(PolarsProcessor.SSOT_SCHEMA[c]) for c in PolarsProcessor.SSOT_SCHEMA.keys()])
    ...
    def calc_from_polars(df_pl: pl.DataFrame, latest_only: bool = True) -> pl.DataFrame:
        ...
        try:
            ...
            df_pl = df_pl.rename({c: "price" for c in ["Close", "price", "Price"] if c in df_pl.columns and c != "price"})
            
            # 日付の正規化 (entry_date への統合)
            date_col = next((c for c in ["entry_date", "Date", "index"] if c in df_pl.columns), None)
            if date_col:
                df_pl = df_pl.with_columns([
                    pl.col(date_col).cast(pl.Utf8).str.slice(0, 10).str.to_date(strict=False).alias("entry_date")
                ])
            else:
            ...
            # 2. テクニカル指標の計算
            # 指標計算に必要な最低限の行に絞る
            df_pl = df_pl.filter((pl.col("price").is_not_null()) & (pl.col("price") > 0))
            if df_pl.is_empty():
                return PolarsProcessor._ensure_resilient_schema(df_pl.clear())
            ...
            df_pl = (
                ...
                .with_columns(PolarsProcessor._get_rsi_expr(window=14, stability_min=30))
                .with_columns(PolarsProcessor._get_macd_expr())
            )

            # トレンドスコア等の派生指標
            df_pl = df_pl.with_columns([
                (((pl.col("price") - pl.col("ma25")) / pl.col("ma25")) * 100).alias("ma_divergence"),
                (
                    (pl.col("ma25") > pl.col("ma75")).fill_null(False).cast(pl.Int32)
                    + (pl.col("price") > pl.col("ma25")).fill_null(False).cast(pl.Int32)
            ...
            df_pl = df_pl.filter(
                (pl.col("entry_date").is_not_null()) & (pl.col("entry_date").dt.weekday() <= 5)
            )
            ...
            return PolarsProcessor._ensure_resilient_schema(pl.DataFrame(schema=PolarsProcessor.SSOT_SCHEMA))

        except Exception as e:
            logger.warning(f"⚠️ Polars native calculation failed: {e}")
            return pl.DataFrame()
    ...
    def calc_technicals_vectorized(df_pandas: pd.DataFrame) -> dict[str, Any]:
        ...
        res_df = PolarsProcessor.calc_batch_technicals_vectorized({"_legacy_wrapper": df_pandas})
        if res_df.is_empty():
            return {}
        # 1行分を辞書で返す (既存テスト互換)
