...
class DuckDBClient:
    ...
    _memory_conn: Optional[duckdb.DuckDBPyConnection] = None
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        with self._lock:
            ...
            logger.info(
                f"DuckDBClient initialized with path: {self.db_path}, limit: {self.memory_limit}"
            )
        ...
        try:
            ...
            if self.db_path == ":memory:":
                if not hasattr(self, "_memory_conn") or self._memory_conn is None:
                    with self._lock:
                        if (
                            not hasattr(self, "_memory_conn")
                            or self._memory_conn is None
                        ):
                            self._memory_conn = duckdb.connect(
                                self.db_path,
                                config={
