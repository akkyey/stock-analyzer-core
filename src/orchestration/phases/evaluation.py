...
class EvaluationPhase(BasePhase):
    ...
    def execute(self, data_map: Optional[dict] = None) -> Optional[pl.DataFrame]:
        ...
        self.log_info("Executing Deep Financial Repair (Deep Repair #3, Turnaround Detection)...")
        processed_df = FinancialRepairService.apply_deep_repair(processed_df)

        # 3. DuckDB への永続化 (修復済みの全データを保存)
        self.log_info(f"Upserting {len(processed_df)} records to DuckDB for persistence...")
        ...
        # 結合前処理: 最新の計算対象スナップショット（最新日のみ）を抽出
        latest_processed = processed_df.filter(pl.col("entry_date") == pl.col("entry_date").max().over("code"))
        
        # [v12] Schema Isolation: 既存のスコアリング系カラムを物理削除し衝突を防ぐ
        # Acquisition層由来のプレースホルダや、他戦略の残骸をクレンジング
        cols_to_purge = ["quant_score", "strategy_name", "rank", "cnt", "trend_score", "trend_signal"]
        latest_processed_clean = self._clean_columns(latest_processed, cols_to_purge)
        
        # 結合キーの正規化（Date型強制）
        final_scores = final_scores.with_columns([
        ...
        self.log_info(f"Evaluation completed. {len(final_df)} candidates identified, {len(dossiers)} dossiers created.")
    ...
    def _prepare_input_data(self, data_map: Optional[dict] = None) -> Optional[pl.DataFrame]:
        """計算対象のデータを準備する"""
        ...
        if not data_map:
            ...
            return self.context.duck_repo.load_metrics(days=365)

        self.log_info(f"Calculating technicals for {len(data_map)} stocks...")
        return PolarsProcessor.calc_batch_technicals_vectorized(data_map, latest_only=False)
    ...
    def _run_multi_strategy_scoring(self, df: pl.DataFrame) -> list[pl.DataFrame]:
        ...
        strategies = list(self.context.config.get("strategies", {}).keys()) or ["Balanced Strategy"]
