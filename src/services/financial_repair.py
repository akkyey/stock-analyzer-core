...
class FinancialRepairService:
    ...
    def repair(df: pl.DataFrame) -> pl.DataFrame:
        ...
        if "equity_ratio" in cols and "debt_equity_ratio" in cols:
            df = df.with_columns([
                pl.when(pl.col("equity_ratio").is_null() & pl.col("debt_equity_ratio").is_not_null() & (pl.col("debt_equity_ratio") > 0))
        ...
        if all(c in cols for c in ["per", "price", "net_profit", "shares_outstanding"]):
            df = df.with_columns([
                ...
                .then(pl.col("price") / (pl.col("net_profit") / pl.col("shares_outstanding")))
        ...
        if all(c in cols for c in ["net_profit", "prev_net_profit"]):
            df = df.with_columns([
                pl.when((pl.col("prev_net_profit") < 0) & (pl.col("net_profit") > 0))
                .then(pl.lit("turnaround_black"))
                .when((pl.col("prev_net_profit") > 0) & (pl.col("net_profit") < 0))
                .then(pl.lit("turnaround_white"))
                .when((pl.col("prev_net_profit") < 0) & (pl.col("net_profit") < 0) & (pl.col("net_profit") > pl.col("prev_net_profit")))
                .then(pl.lit("loss_shrinking"))
                .otherwise(pl.lit("normal"))
                .alias("turnaround_status")
            ])
            df = df.with_columns([
                pl.when(pl.col("turnaround_status") == "turnaround_black").then(1).otherwise(0).alias("is_turnaround")
            ])
        ...
        if all(c in cols for c in ["net_profit", "prev_net_profit"]):
            df = df.with_columns([
                ...
                .then((pl.col("net_profit") - pl.col("prev_net_profit")) / pl.col("prev_net_profit").abs() * 100.0)
                .otherwise(None)
        ...
        if "per" in cols:
            df = df.with_columns([
                pl.when(pl.col("per") > 1000.0).then(1000.0).otherwise(pl.col("per")).alias("per")
            ])
        if "equity_ratio" in cols:
            df = df.with_columns([
                pl.col("equity_ratio").clip(lower_bound=0.0, upper_bound=100.0).alias("equity_ratio")
            ])

        # [Step 6] 比率項目の自動スケーリング (#4)
        # [v27.3] 閾値（デフォルト10.0）未満（実数表記）の場合、百分率へ 100倍スケーリングする
        from src.config_singleton import ConfigSingleton
        threshold = ConfigSingleton.get("financial_repair.ratio_scaling_threshold", 10.0)

        ratio_cols = [c for c in ["current_ratio", "quick_ratio", "equity_ratio", "debt_equity_ratio"] if c in cols]
        if ratio_cols:
            df = df.with_columns([
        ...
        logger.info(f"✨ Deep financial repair completed. (Records: {initial_count}, Scaling Threshold: {threshold})")
        return df
