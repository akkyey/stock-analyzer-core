...
def test_get_top_results(db_conn):
    """スコア上位取得のテスト"""
    repo = AnalysisRepository()
    repo.save({"code": "1", "strategy_name": "S", "quant_score": 50, "analyzed_at": get_current_time(), "row_hash": "h1"})
    repo.save({"code": "2", "strategy_name": "S", "quant_score": 90, "analyzed_at": get_current_time(), "row_hash": "h2"})
    repo.save({"code": "3", "strategy_name": "S", "quant_score": 70, "analyzed_at": get_current_time(), "row_hash": "h3"})
...
def test_clear(db_conn):
    """分析結果のクリア機能のテスト"""
    repo = AnalysisRepository()
    repo.save({"code": "1", "strategy_name": "A", "analyzed_at": get_current_time(), "row_hash": "h1"})
    repo.save({"code": "2", "strategy_name": "B", "analyzed_at": get_current_time(), "row_hash": "h2"})
...
def test_delete_by_codes_with_filters(db_conn):
    ...
    repo.save({"code": "1001", "analyzed_at": now_time, "row_hash": "h1", "ai_reason": "Real results"})
    repo.save({"code": "1002", "analyzed_at": now_time, "row_hash": "h2", "ai_reason": "[MOCK] test"})
