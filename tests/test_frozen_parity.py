...
def test_rsi_stability_and_null_propagation(mock_context):
    ...
    res_short = PolarsProcessor.calc_batch_technicals_vectorized(data_short, latest_only=True)
    res_long = PolarsProcessor.calc_batch_technicals_vectorized(data_long, latest_only=True)
...
def test_holiday_purge_in_processor():
    ...
    # あるいは全量データを返して件数を見る
    res = PolarsProcessor.calc_batch_technicals_vectorized({"7777": df_pandas}, latest_only=False)
    
    # 4件中、土日が消えて 2件になっているはず
    assert len(res) == 2
    # 日付が金曜と月曜であることを確認
    days = [d.strftime("%Y-%m-%d") if hasattr(d, "strftime") else str(d) for d in res["entry_date"].to_list()]
...
def test_early_memory_join_scoring(mock_context):
    ...
    stock_repo.upsert([{"code": "8888", "name": "Test Stock", "sector": "Information", "market": "TSE"}])
    funda_repo.upsert([{
        "code": "8888", 
        "per": 15.0, 
        "pbr": 1.2, 
