...
def test_polars_processor_batch_technicals():
    ...
    # 実行
    result_df = PolarsProcessor.calc_batch_technicals_vectorized(hist_map, latest_only=True)
...
def test_polars_processor_missing_code_column():
    ...
    df_no_code = pl.DataFrame({"Close": [100.0, 110.0], "Date": [pd.Timestamp("2026-01-01"), pd.Timestamp("2026-01-02")]})
