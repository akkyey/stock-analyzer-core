...
from tests.helpers.stubs import StubOrchestratorContext, DataGenerator
...
def test_acquisition_phase_execute_success(stub_context):
    ...
    with patch("src.repositories.duck_repository.DuckDBRepository") as MockDuckRepo, \
         patch("src.repositories.market_data_repository.MarketDataRepository") as MockMarketRepo, \
        ...
        mock_market.get_all_history_pl.return_value = pl.DataFrame(schema={"code": pl.Utf8, "Date": pl.Datetime})
        
        mock_fetcher = MockFetcher.return_value
        # yfinance データの代わり
        hist_1001 = pd.DataFrame({"Close": [100, 105], "Volume": [1000, 1100]}, index=pd.to_datetime(["2026-04-01", "2026-04-02"]))
        hist_1002 = pd.DataFrame({"Close": [200, 190], "Volume": [500, 600]}, index=pd.to_datetime(["2026-04-01", "2026-04-02"]))
        mock_fetcher.fetch_stock_data.return_value = {"1001": hist_1001, "1002": hist_1002}
