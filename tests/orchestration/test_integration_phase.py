...
def test_integration_phase_execute_success(stub_context):
    ...
    with patch("os.path.exists") as MockExists, \
         ...
         patch("src.orchestration.report_helper._upload_summary_to_gspread") as MockGspread:
...
def test_integration_phase_upload_failure(stub_context):
    ...
    with patch("os.path.exists", return_value=True), \
         ...
         patch("src.colab_tools.ColabTools.upload_file_to_drive", side_effect=Exception("Upload Error")), \
        ...
        assert stub_context.notifier.notify_success.called or stub_context.notifier.notify_error.called
