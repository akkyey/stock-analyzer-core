...
def engine_config():
    return {
        "strategies": {
            "Balanced": {
                "base_score": 50.0,
                "points": {"roe": 10.0, "dividend_yield": 5.0},
                "thresholds": {"roe": 10.0},
                "metrics_metadata": {
...
def test_polars_engine_advanced_scoring(engine_config):
    ...
    config["strategies"]["Balanced"]["base_score"] = (
        10.0  # 大幅に下げて合計が 100 に達するのを防ぐ
    )
...
def test_polars_engine_filter_candidates():
    ...
    candidates = PolarsEngine.filter_candidates(
        df, min_price=50.0, min_volume=1000.0, min_equity_ratio=10.0, max_candidates=10
    )
