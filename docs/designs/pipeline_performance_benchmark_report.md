...
> [!NOTE] 計測環境と対象データセットの整合性について
> 本ドキュメントに記載されている処理時間（シナリオA: 3.31秒、シナリオB: 87.62秒など）および出力社数（スクリーニング通過: 2,120社、除外・計算不能: 1,800社、計3,920社）は、すべてリモート実機サーバー `masaaki-sv`（Linux Ubuntu x86_64, Python 3.12.8）上の同一蓄積データセット（全3,920社）を対象とした実測値です。ローカル開発環境（macOS）はコード編集および同期元として使用しており、計測値の混在はありません。
...
> [!NOTE] 計測データセットについて
> 本計測は、リモート実機サーバー `masaaki-sv` 上の DuckDB / SQLite ローカル蓄積データ（東証全3,920社）を用いて実施された実測値です。出力社数（2,120社 / 1,800社）も本計測環境において同一データセットから直接生成・検証された確定値です。

| フェーズ | 処理内容 | 最新実測所要時間 (2026-09-23) | 時間割合 (%) | 処理規模・スループット |
| :--- | :--- | :---: | :---: | :--- |
| **0. DB初期化** | DuckDB & SQLite の全テーブルクリア・再作成 | **76.06 ms** | 2.30 % | テーブル DROP / CREATE |
...
本検証では、**「同一の直近30日スキャン」における2つの極端な状態**を実測した。

| 運用状態・測定モード | Phase 1: EDINET差分 | Phase 2: yfinance株価取得 | Phase 3: 3層クオンツ分析 | 総合パイプライン完遂時間 | 備考・通信挙動 |
| :--- | :---: | :---: | :---: | :---: | :--- |
| **【状態 A: 最大負荷DL時】**<br>(キャッシュ無効化・全件新規取得) | **3分 10.2 秒**<br>(190.2 秒) | **1分 58.2 秒**<br>(118.2 秒) | **0.15 秒**<br>(147 ms) | **5分 08.5 秒**<br>(308.5 秒) | 188社すべての有報ZIPを実通信DL。全3,920社のyfinance株価取得と3層分析を完走（※通信時間は 2026-09-22〜09-23 実測値） |
| **【状態 B: 平常差分同期時】**<br>(手元DB最新化済み・キャッシュ有効) | **0.58 秒** | **1分 39.6 秒**<br>(99.6 秒) | **0.15 秒**<br>(147 ms) | **1分 40.3 秒**<br>(100.3 秒) | 新着差分のみ同期(再DLスキップ)。平常運用時の実測値（Phase 1・Phase 3は2026-09-24再計測値、Phase 2は2026-09-23実測値） |

> [!NOTE]
> **【実通信計測値の採用基準に関する注記】**
> - 外部API（金融庁EDINETおよびYahoo! Finance）への短時間での集中アクセスによる過剰負荷やアクセス制限（HTTP 429 Too Many Requests）を回避するため、実通信を伴うフェーズ（状態Aの全工程、および状態BのPhase 2）は、2026-09-22〜09-23 に実機サーバーで計測された実測値をそのまま採用している。
> - 一方、最新のコード改修（二層キャッシュ判定の厳密化、PreFilterの財務欠損隔離、Polars 3層評価）が直接影響する **状態BのPhase 1（0.58秒）** および **Phase 3（0.15秒 / 147 ms）** は、2026-09-24 に最新の実機環境で再計測した確定値を反映している。

#### 3. 結論
* **「平常時は 約1分40秒 で超高速に完走しつつ、万が一の停止や修正に対しては 0.58秒 の極小コストで30日間の安全網を張る」**
