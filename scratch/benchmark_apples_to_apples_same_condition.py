...
【計測フェーズ】:
Phase 1: EDINET 直近30日間 差分バックフィル (前回: 7分42秒)
...
logger = logging.getLogger("ApplesToApplesBench")


def run_benchmark():
    print("=" * 85, flush=True)
    print("🍏 【前回Qiita記事と完全同条件】 全パイプライン性能実測ベンチマーク", flush=True)
    print(f"   実行日時: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    print(f"   条件設定: EDINET スキャン日数 = 30 日間 (直近差分バックフィル)", flush=True)
    print(f"   比較基準: 前回記事記録 ➔ Phase 1: 7分42秒 | Phase 2: 91秒 | Total: 9分13秒", flush=True)
    print("=" * 85, flush=True)
    ...
    # =========================================================================
    # Phase 1: EDINET 直近30日間 差分バックフィル (前回記録: 7分42秒)
    # =========================================================================
    print("\n" + "=" * 60, flush=True)
    print("📡 【Phase 1】 EDINET 直近30日間 差分バックフィル ＆ DB更新", flush=True)
    print("=" * 60, flush=True)
    ...
    # 30日分の書類スキャン ＆ 差分ダウンロード・パース
    # ※レジューム（すでに取得済みの成果物はスキップ）で新着差分のみを処理
    print("  -> 直近30日間の開示書類スキャン開始 (days=30)...", flush=True)
    edinet_results = manager.run_turbo_acquisition(days=30)
    print(f"  ✅ EDINET 取得・パース完了: 対象 {len(edinet_results):,} 銘柄", flush=True)
    ...
    print(f"  ✅ [Phase 1 完了] 直近30日 差分バックフィル所要時間: {timings['p1_edinet_backfill']:.2f} 秒 ({timings['p1_edinet_backfill']/60:.2f} 分)", flush=True)
    ...
    # =========================================================================
    print("\n" + "=" * 85, flush=True)
    print("🏆 【前回Qiita記事 vs 今回実測】 完全同条件ベンチマーク対比結果", flush=True)
    print("=" * 85, flush=True)
    print(f"{'測定フェーズ':<32} | {'前回Qiita記事 (2026-03)':<22} | {'今回実測 (2026-09 最新DB)':<22}", flush=True)
    print("-" * 85, flush=True)
    print(f"{'Phase 1: EDINET 30日差分バックフィル':<28} | {'7分42秒 (462 秒)':<20} | {f'{int(timings["p1_edinet_backfill"]//60)}分{timings["p1_edinet_backfill"]%60:.1f}秒 ({timings["p1_edinet_backfill"]:.1f}秒)':<20}", flush=True)
    print(f"{'Phase 2: 全銘柄分析・順位付け':<28} | {'1分31秒 ( 91 秒)':<20} | {f'{int(timings["p2_analysis"]//60)}分{timings["p2_analysis"]%60:.1f}秒 ({timings["p2_analysis"]:.1f}秒)':<20}", flush=True)
    print("-" * 85, flush=True)
    ...
    print("=" * 85, flush=True)
    print(f"✅ 生成レポート:", flush=True)
    print(f"   - 評価可能銘柄: {daily_path} ({len(eval_df):,} 社)", flush=True)
    print(f"   - 除外銘柄:     {uncalc_path} ({len(rejected_pre_df):,} 社)", flush=True)
    print("=" * 85, flush=True)
