"""全3,900銘柄 外部API実通信フルパイプライン 性能実測スクリプト

東証全上場銘柄（約3,900社）を対象に、実際に外部API（JPX、EDINET、yfinance）と通信し、
1. データベース初期化
2. 外部APIデータ取得（全銘柄の株価OHLCV並列取得）
3. DB構築・永続化
4. 財務データ補完 (Financial Deep Repair)
5. 分類・多層評価・2ファイル直接分離出力
の全工程の実測時間を精密に計測する。
"""
...
def run_full_live_benchmark():
    print("=" * 80)
    print("🚀 【東証全3,900銘柄 外部API実通信フルパイプライン 実測計測開始】")
    ...
    bench_sqlite_path = os.path.join(ROOT_DIR, "data/stock_master_live_full.db")
    ...
    # -------------------------------------------------------------
    # 1. 外部APIデータ取得 (External API Live Acquisition: 全3,900銘柄)
    # -------------------------------------------------------------
    print("\n[Phase 1] 外部APIデータ取得 (External API Live Acquisition) 開始...")
    t1_start = time.perf_counter()

    # 1-1. JPX公式Webサーバー通信 (最新上場銘柄リスト Excel)
    ...
    res_jpx = requests.get(jpx_url, timeout=20)
    ...
    # 1-3. Yahoo! Finance (yfinance) 全3,900銘柄並列ダウンロード
    # 全銘柄リストの準備
    df_jpx_local = pl.read_csv(os.path.join(ROOT_DIR, "data/input/jp_stock_list.csv")).unique(subset=["code"])
    all_codes = df_jpx_local["code"].to_list()
    total_target = len(all_codes)

    print(f"  📡 (3/3) Yahoo! Finance ライブ通信 (東証全 {total_target:,} 銘柄, 直近5営業日 OHLCV)...")
    yf_start = time.perf_counter()

    CHUNK_SIZE = 500
    code_chunks = [all_codes[i : i + CHUNK_SIZE] for i in range(0, total_target, CHUNK_SIZE)]
    print(f"     -> チャンク分割: {len(code_chunks)} バッチ (1バッチあたり最大 {CHUNK_SIZE} 銘柄)")
    ...
    for idx, chunk in enumerate(code_chunks, start=1):
        ...
        yf_chunk_data = yf.download(tickers, period="5d", progress=False, threads=True)
        chunk_elapsed = time.perf_counter() - chunk_t0
        speed = len(chunk) / chunk_elapsed if chunk_elapsed > 0 else 0

        print(f"     [{idx}/{len(code_chunks)}] {len(chunk)} 銘柄取得完了: {chunk_elapsed:.2f} 秒 ({speed:.1f} 銘柄/秒)")

        # フラット化
        if hasattr(yf_chunk_data, "columns") and hasattr(yf_chunk_data.columns, "levels") and len(yf_chunk_data.columns.levels) > 1:
            close_df = yf_chunk_data["Close"]
            vol_df = yf_chunk_data["Volume"]
            for t in tickers:
                code = t.replace(".T", "")
        ...
        # チャンク間の安全待機（APIレート制限対策）
        if idx < len(code_chunks):
            time.sleep(0.3)

    sub_timings_api["yfinance_download"] = time.perf_counter() - yf_start
    total_speed = total_target / sub_timings_api["yfinance_download"]
    print(f"     ✅ yfinance 全 {total_target:,} 銘柄ダウンロード完了: {sub_timings_api['yfinance_download']:.2f} 秒 ({total_speed:.1f} 銘柄/秒)")
    ...
    print(f"     - yfinance 通信: {sub_timings_api['yfinance_download']:.3f} 秒")
    print(f"     - 取得市況レコード総数: {len(df_live_ts):,} 行")
    ...
    with sqlite3.connect(bench_sqlite_path) as s_conn:
        df_jpx_local.to_pandas().to_sql("stocks", s_conn, if_exists="append", index=False)
        if not df_live_ts.is_empty():
            df_live_ts.to_pandas().to_sql("market_data", s_conn, if_exists="append", index=False)
        s_cursor = s_conn.cursor()
        s_cursor.execute("CREATE INDEX idx_live_md_code ON market_data(code, entry_date)")
        s_conn.commit()
    ...
    total_db_records = len(df_jpx_local) + len(df_live_ts)
    ...
    df_candidates = df_repaired.join(df_jpx_local.select(["code", "name", "sector", "market"]), on="code", how="left")
    ...
    # -------------------------------------------------------------
    print("\n" + "=" * 80)
    print("📊 【全3,900銘柄 外部API実通信パイプライン ベンチマーク実測結果サマリー】")
    print("=" * 80)
    ...
    print(f"{'1. 外部APIデータ取得 (Acquisition)':<20} | {p1:>8.3f} 秒  | {(p1/total_time)*100:>8.2f} % | 【実通信】全3,900社 yfinance + JPX + EDINET")
    ...
    print(f"   └─ yfinance 全銘柄並列取得    | {sub_timings_api['yfinance_download']:>8.3f} 秒  |       -    | 全{total_target:,}社 ({total_speed:.1f} 銘柄/秒)")
    print(f"{'2. DB構築 (DB Ingestion)':<24} | {p2:>8.3f} 秒  | {(p2/total_time)*100:>8.2f} % | {total_db_records:,} 行バルク格納・インデックス作成")
    print(f"{'3. 補完 (Financial Repair)':<23} | {p3*1000:>8.2f} ms | {(p3/total_time)*100:>8.2f} % | 全3,900社 ベクトル補完・PER逆算")
    print(f"{'4. 分類 (Classification/PreFilter)':<20} | {p4*1000:>8.2f} ms | {(p4/total_time)*100:>8.2f} % | 3層評価 & 2ファイル完全分離出力")
    print("-" * 80)
    print(f"{'合計 (Total Pipeline Time)':<26} | {total_time:>8.3f} 秒  | 100.00 % | 全3,900銘柄の外部実通信を含む全工程完了")
    print("=" * 80)


if __name__ == "__main__":
    run_full_live_benchmark()
