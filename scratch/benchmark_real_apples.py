"""
前回Qiita記事（第4弾・完結編）と完全同一条件での性能実測スクリプト
【キャッシュ無効化・完全実通信ダウンロード＆パース版】

【計測フェーズ】:
Phase 1: EDINET 直近30日間 差分バックフィル (実通信DL＆パース) (前回: 7分42秒 [351社])
Phase 2: 東証全銘柄 最新株価ライブ取得 ＆ 3層クオンツ分析・2ファイル分離出力 (前回: 91秒)
Total: 運用サイクル総合完遂時間 (前回: 9分13秒)
"""
...
logger = logging.getLogger("RealBenchApples")


def run_benchmark():
    print("=" * 85, flush=True)
    print("🍏 【真の同条件実測: キャッシュ完全無効化】 全パイプライン性能実測ベンチマーク", flush=True)
    print(f"   実行日時: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    print(f"   条件設定: EDINET スキャン日数 = 30 日間 (直近新着・差分実通信DL & パース)", flush=True)
    print(f"   比較基準: 前回記事記録 ➔ Phase 1: 7分42秒 (351社) | Phase 2: 91秒 | Total: 9分13秒", flush=True)
    print("=" * 85, flush=True)
    ...
    # =========================================================================
    # Phase 1: EDINET 直近30日間 差分実通信DL＆パース (前回記録: 7分42秒)
    # =========================================================================
    print("\n" + "=" * 60, flush=True)
    print("📡 【Phase 1】 EDINET 直近30日間 差分バックフィル (実通信DL & パース)", flush=True)
    print("=" * 60, flush=True)
    ...
    # 重要: キャッシュを完全無効化し、前回の記事と同様に実際に金融庁からDL＆パースを行う
    clean_bench_results_dir = os.path.join(ROOT_DIR, "data/tmp/bench_real_dl_results")
    clean_bench_xbrl_dir = os.path.join(ROOT_DIR, "data/tmp/bench_real_dl_xbrl")
    if os.path.exists(clean_bench_results_dir):
        shutil.rmtree(clean_bench_results_dir)
    if os.path.exists(clean_bench_xbrl_dir):
        shutil.rmtree(clean_bench_xbrl_dir)
    os.makedirs(clean_bench_results_dir, exist_ok=True)
    os.makedirs(clean_bench_xbrl_dir, exist_ok=True)

    manager.results_dir = clean_bench_results_dir
    manager.tmp_dir = clean_bench_xbrl_dir

    print("  -> キャッシュ完全無効化: 金融庁 EDINET から実際に有報ZIPをダウンロード開始...", flush=True)
    edinet_results = manager.run_turbo_acquisition(days=30)
    print(f"  ✅ EDINET 実通信DL ＆ パース完了: {len(edinet_results):,} 銘柄", flush=True)
    ...
    print(f"  ✅ [Phase 1 完了] 直近30日 差分実通信DL＆DB更新所要時間: {timings['p1_edinet_backfill']:.2f} 秒 ({timings['p1_edinet_backfill']/60:.2f} 分)", flush=True)

    # =========================================================================
    # Phase 2: 最新株価取得 ＆ 3層クオンツ分析・2ファイル分離出力 (前回記録: 91秒)
    # =========================================================================
    print("\n" + "=" * 60, flush=True)
    ...
    # 2. yfinance による最新株価並列バッチ取得 (500銘柄/チャンク)
    ...
    # 第1層 PreFilter
    pf_res = PreFilter.evaluate(df_repaired)
    passed_df = pf_res.passed_df
    rejected_pre_df = pf_res.rejected_df

    # 第2層 & 第3層 QuantAgentEvaluator
    dossiers = StockDossierBuilder.from_dataframe(passed_df, limit=None)
    agent_results = []
    for d in dossiers:
        score, verdict, thesis, triggers = QuantAgentEvaluator.evaluate(d)
        agent_results.append({
            ...
            "price": d.get("price"),
            "rsi_14": d.get("rsi_14"),
            "macd_status": "Bullish" if (d.get("macd_hist") or 0) > 0 else "Bearish",
            "ma_divergence": d.get("ma_divergence"),
            "per": d.get("per"),
            "pbr": d.get("pbr"),
            "roe": d.get("roe"),
    ...
    eval_df = pl.DataFrame(agent_results) if agent_results else pl.DataFrame()
    if not eval_df.is_empty():
        eval_df = eval_df.sort("agent_score", descending=True).with_columns(
    ...
    if not eval_df.is_empty():
        ...
        eval_df.to_pandas().to_csv(daily_path, mode="a", index=False)

    if not rejected_pre_df.is_empty():
        with open(uncalc_path, "w", encoding="utf-8") as f:
            f.write(gen_time_comment)
        rejected_pre_df.to_pandas().to_csv(uncalc_path, mode="a", index=False)
    ...
    # 最終対比レポート出力
    # =========================================================================
    p1_min = int(timings['p1_edinet_backfill'] // 60)
    p1_sec = timings['p1_edinet_backfill'] % 60
    p1_str = f"{p1_min}分{p1_sec:.1f}秒 ({timings['p1_edinet_backfill']:.1f}秒)"

    p2_min = int(timings['p2_analysis'] // 60)
    p2_sec = timings['p2_analysis'] % 60
    p2_str = f"{p2_min}分{p2_sec:.1f}秒 ({timings['p2_analysis']:.1f}秒)"

    tot_min = int(timings['total'] // 60)
    tot_sec = timings['total'] % 60
    tot_str = f"{tot_min}分{tot_sec:.1f}秒 ({timings['total']:.1f}秒)"

    print("\n" + "=" * 85, flush=True)
    print("🏆 【真の同条件対比: 実通信DL＆パース】 ベンチマーク対比結果", flush=True)
    print("=" * 85, flush=True)
    print(f"{'測定フェーズ':<32} | {'前回Qiita記事 (351社DL)':<22} | {'今回実測 (197社実通信DL)':<22}", flush=True)
    print("-" * 85, flush=True)
    print(f"{'Phase 1: 差分実通信DL＆パース':<28} | {'7分42秒 (462 秒)':<20} | {p1_str:<20}", flush=True)
    print(f"{'Phase 2: 全銘柄分析・順位付け':<28} | {'1分31秒 ( 91 秒)':<20} | {p2_str:<20}", flush=True)
    print("-" * 85, flush=True)
    print(f"{'総合パイプライン合計 (Total)':<28} | {'9分13秒 (553 秒)':<20} | {tot_str:<20}", flush=True)
    print("=" * 85, flush=True)
    count_dl = max(1, len(edinet_results))
    sec_per_stock = timings['p1_edinet_backfill'] / count_dl
    print(f"📊 1社あたり実通信DL＆パース平均速度:", flush=True)
    print(f"   - 前回: 462秒 / 351社 = 約 1.31 秒 / 社", flush=True)
    print(f"   - 今回: {timings['p1_edinet_backfill']:.1f}秒 / {len(edinet_results)}社 = 約 {sec_per_stock:.2f} 秒 / 社", flush=True)
    print("=" * 85, flush=True)
